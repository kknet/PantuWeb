...
class BasicDB extends \PDO
{
    ...
    public function __construct($charset = 'utf8')
    {
				...
				try {
				            parent::__construct('mysql:host=' .Config::$DBhostname . ';dbname=' .Config::$DBdatabasename, Config::$DBusername, Config::$DBpassword );
				            $this->dbName = Config::$DBdatabasename;
    ...
    public function select($columns)
    {
		...
		public function customWhereDate($where, $date = '')
    {

			switch ($date) {
			            case 'today':
			                $query = "WHERE DATE(`".$where."`) = CURDATE()";
			                break;
			            case 'yesterday':
			                $query = "WHERE ".$where." = DATE_SUB(CURDATE(),INTERVAL 1 DAY)";
			                break;
			            case 'this_week':
			                $query = "WHERE WEEK(".$where.", 1) = WEEK(CURDATE(), 1)";
			                break;
			            case 'week':
			                $query = "WHERE ".$where." >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK)";
			                break;
			            case 'month':
			                $query = "WHERE ".$where." >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)";
			                break;
			            case 'all':
			                $query = "";
			                break;
